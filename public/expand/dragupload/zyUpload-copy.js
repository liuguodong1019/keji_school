		...
		return this.each(function(){
			...
			this.createHtml = function(){
				...
				if(para.dragDrop){
					...
					html += '		<ul class="upload_file_list">';
					html += '			<li class="upload_file_item title">';
					html += '				<span class="file_item_name ming">文件名</span>';
					html += '				<span class="file_item_size daxiao">大小</span>';
					html += '				<span class="file_item_jindu zh_tai">状态</span>';
					html += '				<span class="file_item_ cha"></span>'
					html += '			</li>';
					html += '		</ul>';

					html += '		<div class="upload_main"><input type="hidden" value="'+item_id+'" name="item_id">';
					html += '			<span id="fileDragArea" class="upload_drag_area">将文件拖到此处上传<div id="preview" class="upload_preview"></div></span>';
					html += '		</div>';
					...
					html += '					<div class="btns">';
	            	html += '				<div class="webuploader_pick btn btn-primary">添加文件</div>';
	            	html += '				<div class="upload_btn btn btn-primary">开始上传</div>';
	            	html += '			</div>';
            		html += '					<input id="fileImage" type="file" size="30" name="fileselect" '+multiple+'>';
            		html += '				</div>';
            		html += '			</div>';
					html += '			';
		            html += '			<div class="file_total_zize">';
		            html += '				<span id="f_total_zize" totals="0"></span>';
		            html += '				<span id="f_total_jd">0%</span>';
		            html += '			</div>';
					html += '		</div>';
			...
			this.funSetStatusInfo = function(files){
				...
				/*var arrfilesize=[];
				$.each(files, function(k,v){
					arrfilesize[k]=v.size
					//console.info(k)  //文件序号
					//console.info(v.size) 
					console.info(arrfilesize)
				});*/
				...
				for (var i = 0, file; file = files[i]; i++){
					arrfilesize22[i]=files[i].size;
					arrfilename22[i]=files[i].name;
					arrfiletype22[i]=files[i].type;
				}
				console.info(arrfilesize22)
				console.info(arrfilename22)
				console.info(arrfiletype22)
			...
			this.funDisposePreviewHtml = function(file, e){
				...
				// 处理不同类型文件代表的图标
				var fileImgSrc = base_url+"public/expand/dragupload/control/images/fileType/";
			...
			this.createCorePlug = function(){
				var params = {
					...
					onSelect: function(selectFiles, allFiles) {
						...
						var funAppendPreviewHtml = function(html){
							...
							if(para.dragDrop){
								$("#preview").append(html);
								var total_ssss=0;
								var nnnn=parseInt($('#f_total_zize').attr('totals'));
								console.log("nnnn:",nnnn)
								total_ssss=total_ssss+nnnn;
								for (var i = 0, file; file = selectFiles[i]; i++){
									// 转化为kb和MB格式。
									var ssss=0;
									if(selectFiles[i].size > 1024 * 1024) {                    
										ssss = (Math.round(selectFiles[i].size * 100 / (1024 * 1024)) / 100).toString() + 'MB'; 
										console.info(111) 
										console.info(selectFiles[i].size)              
									}else{  
										console.info(222,selectFiles[i].size)                  
										ssss = (Math.round((selectFiles[i].size) * 100 / 1024) / 100).toString() + 'KB'; 
										console.info(222,selectFiles[i].size)                
									} 
									var addHtml='';
									addHtml += '		<li class="upload_file_item">';
									addHtml += '			<span class="file_name ming">' + selectFiles[i].name + '</span>';
									addHtml += '			<span class="file_size daxiao">' + ssss + '</span>';
									addHtml += '			<span class="file_jindu zh_tai">0%</span>';
									addHtml += '			<span class="file_del cha" data-index="'+i+'" title="删除">×</span>'
									addHtml += '		</li>';
									$('.upload_file_list').append(addHtml)
									//获取文件总大小
									total_ssss += selectFiles[i].size;
								}
									//console.info("选择的文件大小",total_ssss)
								//获取文件总大小
								$('#f_total_zize').attr('totals',total_ssss)
								if(total_ssss > 1024 * 1024) {                    
										total_ssss = (Math.round(total_ssss * 100 / (1024 * 1024)) / 100).toString() + 'MB'; 
										console.info(333,total_ssss)              
									}else{  
										total_ssss = (Math.round((total_ssss) * 100 / 1024) / 100).toString() + 'KB'; 
										console.info(444,total_ssss)                
									} 
								$('#f_total_zize').text(total_ssss)
						...
						var funBindDelEvent = function(){
							if($(".file_del").length>0){
								...
								$(".file_del").click(function() {
									...
									//预览框删除的同时，删除上方列表的对应内容
									if($(this).parents('.upload_file_item').length>0){
										//console.info("yes:点击的上方列表 x")
										$(this).parents('.upload_file_item').remove();
									}else{
										var nn=$(this).attr("data-index");
										//console.info("no:点击的下方列表 x",nn)
										$('.upload_file_item .file_del[data-index='+nn+']').parents('.upload_file_item').remove();
									}
									return false;	
								});
							}
					...
					onProgress: function(file, loaded, total) {
						...
						//上传时 百分比数据动态显示
						var percent02 = (loaded / total * 100).toFixed(0) + '%'
						$('.upload_file_list .upload_file_item').eq(file.index+1).find('.zh_tai').text(percent02)
					},
					onSuccess: function(file, response) { //此处为点击“开始上传” 后的操作
						
						$("#uploadProgress_" + file.index).show();//上传成功后，进度条依旧显示(ps:changeto show)
						$("#uploadSuccess_" + file.index).show(); //右下角显示对勾表示上传成功
						if(response>0){
							$("#uploadSuccess_" + file.index).after("<p style='position:absolute;width:100%;text-align:center;'>上传成功</p>") //(ps:add)
							$("#uploadInf").append("<p style='background:white;'>上传成功</p>");
						}else{
							$("#uploadInf").append("<p style='background:white;color:red;'>"+response+"</p>");
						}
						// 根据配置参数确定隐不隐藏上传成功的文件
						if(para.finishDel){
							// 移除效果
							//$("#uploadList_" + file.index).fadeOut();		//(ps:注释掉了)
							// 重新设置统计栏信息
							self.funSetStatusInfo(ZYFILE.funReturnNeedFiles());
						}
						$("#uploadList_" + file.index).show(); //上传后原位置依旧显示缩略图(ps:add)
					},
